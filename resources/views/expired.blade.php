...
@push('page-header')
<div class="col-sm-12">
	<h3 class="page-title">{{__(trans('product.expired'))}}</h3>
	<ul class="breadcrumb">
		<li class="breadcrumb-item"><a href="{{route('products')}}">{{__(trans('product.products'))}}</a></li>
		<li class="breadcrumb-item active">{{__(trans('product.expired'))}}</li>
	</ul>
</div>
@endpush
@section('content')
					...
					<table id="datatable-export" class="table table-striped table-bordered table-hover table-center mb-0">
						...
						<tbody>
							...
							<tr>
								...
								<td><span class="btn btn-sm bg-danger-light">{{__(trans('product.the_product_is_expired'))}}</span></td>
								<td>
									<div class="actions">
